...
impl TimerBot {
    ...
    pub fn commands() -> Commands<Self> {
        Commands {
            commands: vec![CommandBuilder::new().literal(["!timer"]).split([
                CommandBuilder::<Self, _>::new()
                ...
                CommandBuilder::new()
                    ...
                    .split([
                        CommandBuilder::<Self, _>::new().word().finalize(
                            true,
                            ...
                            Arc::new(|bot, _, args| {
                                match Timer::parse_duration(args[1].as_str()) {
                                    ...
                                    Err(err) => {
                                        Some(format!("Failed to parse time argument: {err}"))
                                    }
                                }
                            }),
                        ),
